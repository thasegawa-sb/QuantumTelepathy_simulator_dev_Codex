...
| A27 | The Phase 7 status consumes effective system-level `epsilon` and `R_HEG`; Phase 8 must derive them after memory decoherence and time multiplexing before a lower-level hardware scenario can claim overall PASS. | Active until the M2 model is connected. |
