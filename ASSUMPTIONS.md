...
| A27 | The Phase 7 status consumes effective system-level `epsilon` and `R_HEG`; these must be derived after memory decoherence and time multiplexing before a lower-level hardware scenario can claim overall PASS. | Fulfilled for the Phase 8 analytical M2 model; Table III parameter derivation remains Phase 9. |
| A28 | Li Eq. 46 uses the paper's deterministic occupancy assumption that the input becomes available when entanglement is ready for measurement. | Active for the analytical M2 oracle; stochastic trigger timing and buffer occupancy distributions are deferred to event-driven cross-validation. |
